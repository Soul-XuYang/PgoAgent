...
**接口文档**：本项目使用**Swagger**进行**GoWeb**接口文档的生成，接口风格为**RESTful**风格
swag init --parseDependency --parseInternal -g main.go -o ./docs 

## 整体的架构图 - Architecture Diagram
可见本文项目的的workflow图
...
## 接口文档生成
这里使用**swagger**进行接口文档的生成，接口风格为**RESTful**风格，注意需要进入到**WebService**目录下执行如下命令：
```bash
swag init --parseDependency --parseInternal -g main.go -o ./docs 
```
或者根据操作系统运行对应的**swagger.bat**或**swagger.sh**文件。
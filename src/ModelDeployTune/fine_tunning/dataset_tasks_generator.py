...
class DatasetGenerator:
    ...
    def _init_parameter_pools(self):
        ...
        self.knowledge_keywords = [
            ...
            "历史", "地理", "英语", "法语", "西班牙语", "日语", "韩语", "俄语", "阿拉伯语", "中文",
        ]
    ...
    def _planner_system_prompt() -> str:
        ...
        return (
            ...
            "能力白名单（capability只能从下列取值，不得发明）：\n"
            "- plan_steps：步骤列表，每个包含：\n"
            "- description：该步骤要做什么（自然语言）\n"
            "- capability：能力类型，选择其一：\n"
            "• none：纯思考/文本生成-无需工具 \n"
            "• list_dir:列出文件及遍历文件目录 \n"
            "• search：本地文件搜索或者部分网络搜索\n"
            "• rag_retrieve：知识库检索执行（LLM会根据问题自动选择合适的检索策略和参数，失败后可调用rag_rewrite_query重写query后重试）\n"
            "• read_file/write_file/create_file/delete_file：文件操作，其中文件中追加内容算入write_file\n"
            "• get_time：时间查询（获取当前时间、日期等实时信息）\n"
            "• calculate：计算操作,包括数学计算、科学计算以及时间计算等\n"
            "• code_exec：shell代码执行,以及python,java,go,c代码的执行\n"
            "• validator: 校验操作，比如校验邮箱、电话号码等"
            "• external_mcp：MCP外部工具，如网络搜索(全网络)、路线规划天气查询等上述未提及的外部工具\n"
            "规则：\n"
            "1) 步骤要原子化、先后清晰，例如：列目录 -> 读文件 -> 整理 -> 回答。\n"
            "2) 只写 capability，不出现具体工具名或实现细节。\n"
            "3) RAG 检索或者重写索引检索都直接用 capability=rag_retrieve\n"
            "4) 仅依据用户最新消息规划，不臆测外部环境。\n"
        )
    ...
    async def generate_planner_samples(
        ...
        planner_llm = llm.with_structured_output(
            Planner, include_raw=True)  # 使用对应的结构进行输出
        system_prompt = self._planner_system_prompt()  # 构建对应的提示词 - 注意自己要写好
    ...
    def generate_identity_samples(self, num_samples: int = 40, model_name: str = "Pgo", origin_model_name="Qwen2.5-3B-Instruct "):
        for _ in tqdm(range(num_samples), desc="生成身份样本"):
            question1 = random.choice(model_identity_templates)
            sample1 = {
                "instruction": "当用户询问你的身份时，直接回答你本身模型名字、职责和能力。",
                "input": question1,
            ...
            sample2 = {
                "instruction": "当用户询问你的作者是谁？或者你是由谁训练或者创建？",
                "input": question2,
                "output": author,
            }
    ...
    async def generate_all(
        ...
        planner_samples: int = 2000,
        test_samples: int = 60,
        custom_name: str = "Pgo",
        origin_model_model: str = "Qwen2.5-3B-Instruct",
    ):
        print("=" * 60)
        print("开始生成多任务数据集!")
        print("=" * 60)
        ...
        await self.generate_planner_samples(planner_samples)
...
async def generate_dataset(output_path: str = "Data.jsonl", random_seed: int = 42, decision_positive_ratio: float = 0.6, planner_concurrency: int = 4, llm_max_retries: int = 3):
    generator = DatasetGenerator(
        output_file=output_path,
        seed=random_seed,
        decision_positive_ratio=decision_positive_ratio,
        planner_concurrency=planner_concurrency,
        planner_retries=llm_max_retries,
    )
    await generator.generate_all(
        decision_samples=1400,
        memory_samples=1600,
        planner_samples=2000,
    )
# ===常量===
scenarios = [
    # 首次建立画像
    ...
    {
        "existing_memory": "[]",
        "conversation_template": "HumanMessage: 你好，我是{profession}，在{province}{city}的{company}工作，钟爱{hobby}。\nAIMessage: 很高兴认识你！",
        "output_template": "- 职业：{profession}\n- 地区：{province}-{city}\n- 公司：{company}\n- 爱好:{hobby}",
    },
    #  不要记（保持不变）
    {
    ...
    {
        "existing_memory": "[]",
        "conversation_template": "HumanMessage: 我朋友是老师，喜欢{hobby}，我也同样喜欢这个爱好。\nAIMessage: 了解。",
        "output_template": "-爱好：{hobby}",
    },
    #   短期状态
    {
        "existing_memory": "- 职业：{profession}\n- 地区：{city}",
...
positive_templates = [
    ...
    "请你帮我查看终端",
    "请你帮我{programming}",
    "请你帮我修改{file}中的{programming}代码",
    "请你帮我查看项目中{file}文件并校验其中文本的邮箱或者电话号码是否正确",
    "校验这个我的学号123456是否正确",
    "请用外部的mcp工具帮我查询下这个明天达到北京的车票多少钱？",
]
...
allowed_capabilities = [  # 共14个操作 - 注意与plan的提示词对应
    "none",
    "list_dir",
    "search",
    "rag_retrieve",
...
model_identity_templates = [
    ...
    "what model are you?",
    "which base model?",
    "你是Qwen2.5-3B-Instruct finetune吗?",
    "你是用来干什么的呢？",
    "你的模型身份是什么？",
    "你具有哪些功能，可以做些什么呢？",
    "你的身份是什么？",
...
if __name__ == "__main__":
    asyncio.run(generate_dataset(planner_concurrency=4,
                output_path="./temp/origin_dataset.jsonl"))

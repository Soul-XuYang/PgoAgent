...
# 设置CUDA设备
os.environ["CUDA_VISIBLE_DEVICES"] = config_loader.get_cuda_visible_devices("embedding")
os.environ["CUDA_DEVICE_ORDER"] = "PCI_BUS_ID"

# 从配置构建vLLM启动命令
vllm_cmd = config_loader.build_vllm_command("embedding")

# 启动vLLM API服务（阻塞运行，等同于直接在终端执行命令）
if __name__ == "__main__":
    try:
        print("=" * 50)
        print(f"开始启动Embedding模型vLLM API服务")
        print(f"   模型：{embedding_config.get('model_name')}")
        print(f"   路径：{embedding_config.get('model_path')}")

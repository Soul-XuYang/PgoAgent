...
# 设置CUDA设备
os.environ["CUDA_VISIBLE_DEVICES"] = config_loader.get_cuda_visible_devices("rerank")
os.environ["CUDA_DEVICE_ORDER"] = "PCI_BUS_ID"

# 从配置构建vLLM启动命令
vllm_cmd = config_loader.build_vllm_command("rerank")

# 启动vLLM API服务（阻塞运行，等同于直接在终端执行命令）
if __name__ == "__main__":
    try:
        print("=" * 50)
        print(f"🚀 启动Rerank模型vLLM API服务")
        print(f"   模型：{rerank_config.get('model_name')}")
        print(f"   路径：{rerank_config.get('model_path')}")
